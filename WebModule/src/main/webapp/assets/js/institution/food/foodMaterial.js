    ...
    getAjax(url,false,getData,function (data) {
        ...
        for (var i = 0; i < num1; i++) {
            html += '<li style="border-top:1px solid #dddddd"><a href="#" class="one"><i class="glyphicon glyphicon-plus"></i>' +
                '' + data.aaData[0][i].meterialTypeName + '</a><cc><a' + ' onclick="openModal(' + data.aaData[0][i].id + ')">添加子类</a>' +
            ...
            if (num2 >= 1) {
                ...
                for (var j = 0; j < num2; j++) {
                    if (data.aaData[0][i].id == data.aaData[1][j].tid) {
                        html += '<li><a href="#" class="one"><i class="glyphicon glyphicon-plus aaa"></i>' + data.aaData[1][j].meterialTypeName + '</a><cc>' +
